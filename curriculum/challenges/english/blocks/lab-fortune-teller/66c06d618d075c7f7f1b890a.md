...
```js
assert.include(__helpers.removeJSComments(code), "Math.random()");
```
...
Your `selectedFortune` variable should correspond to the value of `randomNumber`. For example, if `randomNumber` is 1, `selectedFortune` should be equal to `fortune1`, and so on.
...
```js
assert.deepInclude(spy.calls, [selectedFortune]);
```
