...
Your `p` element should have an opening `<p>` tag.

```js
assert.exists(document.querySelector("p"));
```

Your `p` element should have a closing `</p>` tag.

```js
assert.match(code, /<\/p\s*\>/);
```

Your `p` element's text should be `Learn the skills to become a full-stack developer`. You can copy the text from the instructions.
