...
const { hasJavaScript } = explorer.variables;
assert.isTrue(hasJavaScript?.value.matches('fccSentence.includes("JavaScript")'));
```

Your `hasJavaScript` variable should equal `false`.
