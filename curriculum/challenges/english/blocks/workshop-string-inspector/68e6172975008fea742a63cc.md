...
const { hasLowercaseFCC } = explorer.variables;
assert.isTrue(hasLowercaseFCC?.value.matches('fccSentence.includes("freecodecamp")'));
```

Your `hasLowercaseFCC` variable should equal `false`.
